...
def run(args):  # pylint: disable=C0116
    paths = {p.stem.split('-')[1]: p for p in args.repos.path('mappings').glob('map-*.tsv')}
    translate = {
        'Person/Thing': 'noun',
        'Other': 'other',
        'Number': 'numeral',
    ...
    for language, path in paths.items():
        ...
        for line in reader(path, dicts=True):
            gloss = line['GLOSS'].split('///')[1]
            oc = translate.get(args.repos.conceptsets[line['ID']].ontological_category, 'other')
            cgl = args.repos.conceptsets[line['ID']].gloss
            mappings[language][gloss].add((line['ID'], cgl, int(line['PRIORITY']), oc, 1))

        for gloss in list(mappings[language].keys()):
            if gloss.lower() not in mappings[language]:
                mappings[language][gloss.lower()] = {
                    (x[0], x[1], x[2], x[3], 0) for x in mappings[language][gloss]}
